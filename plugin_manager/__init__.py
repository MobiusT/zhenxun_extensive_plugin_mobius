...
""".strip()
__plugin_des__ = "插件管理"
__plugin_cmd__ = ["下载插件 [_superuser]", "下载依赖 [_superuser]", "更新依赖 [_superuser]"]
...
async def _(arg: Message = CommandArg()):
    ...
    if part_flag:
        ...
        for i in range(7, len(url_list)):
            path += url_list[i] + "/"
        path = path[:-1]
        index_url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}?ref=main"
        try:
            await getfiles(index_url, plugin_folder_path)
...
# 递归下载文件
async def getfiles(url, plugin_folder_path):
    # 获取资源下载信息
    res = await AsyncHttpx.get(url, timeout=30)
    res.encoding = "utf8"
    data = json.loads(res.text)
    for i in data:
        # 检查文件类型，文件夹需要递归下载子文件
        if i["type"] == "dir":
            await getfiles(i["url"], plugin_folder_path)
        elif i["type"] == "file":
            await AsyncHttpx.download_file("https://ghproxy.com/" + i["download_url"],
                                           os.path.join(plugin_folder_path, i["path"]))
        else:
            # 非预期的文件类型跳过处理
...
#下载完整仓库
async def get_repo(owner, repo, plugin_folder_path, plugin_name, branch="master", retry=False):
    #下载链接
    ...
    try:       
        ...
        try:
            ...
            default_branch=re.findall(pattern, str(response.read()))[-1]
            #重试
