...
from nonebot.adapters.onebot.v11 import MessageSegment, Message, GroupMessageEvent, GROUP
...
guess = on_command("崩坏三猜语音", aliases={"崩三猜语音", "崩3猜语音", "崩坏3猜语音", "猜语音"}, priority=5,
                   permission=GROUP, block=True)
answer = on_command("崩坏三猜语音答案", aliases={"崩三猜语音答案", "崩3猜语音答案", "崩坏3猜语音答案", "猜语音答案"},
                    priority=6, permission=GROUP, block=True)
getVoice = on_command("崩坏三语音", aliases={"崩三语音", "崩3语音", "崩坏3语音"}, priority=6, permission=GROUP,
                      block=True)
addAnswer = on_command("崩坏三语音新增答案", aliases={"崩三语音新增答案", "崩3语音新增答案", "崩坏3语音新增答案"},
                       priority=5, permission=SUPERUSER, block=True)
updateAnswer = on_command("崩坏三语音更新答案", aliases={"崩三语音更新答案", "崩3语音更新答案", "崩坏3语音更新答案"},
                          priority=5, permission=SUPERUSER, block=True)
undateVoice = on_command("更新崩坏三语音列表", aliases={"更新崩三语音列表", "更新崩3语音列表", "更新崩坏3语音列表"},
                         priority=5, permission=SUPERUSER, block=True)
...
async def guess_voice(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    # 判断难度
    difficulty = "hard" if re.search(r"2|困难", msg) else "normal"
    gaming = GameSession(event.group_id)
    ret = await gaming.start(difficulty=difficulty)
...
async def check_answer(event: GroupMessageEvent, arg: Message = CommandArg()):
    gaming = GameSession(event.group_id)
    # 游戏未开始返回
    if not gaming.is_start:
        return
    msg = arg.extract_plain_text().strip()
    # 回答是双人时进行符号替换
    msg = msg.lower().replace(",", "和").replace("，", "和")
    await gaming.check_answer(msg, event.user_id)


# 崩坏三语音
@getVoice.handle()
async def send_voice(arg: Message = CommandArg()):
    # sourcery skip: low-code-quality
    msg = arg.extract_plain_text().strip()
    a_list = GameSession.__load__("answer.json")
    assert isinstance(a_list, dict)
    list_cmd = re.compile(r"^列表")  # 命令头
    id_cmd = re.compile(r"\d+$")  # (语音id)
    # 命令包含列表
    if list_cmd.search(msg):
        # 获取要查询的女武神名
        msg = list_cmd.sub('', msg)
        for k, v in a_list.items():
            ...
            if msg in v:
                try:
                    ...
                    return await getVoice.send("语音列表未生成或有错误，请先发送‘更新崩坏三语音列表’来更新")
                text = "".join(
                    f'{i}   {v_list[i]["voice_name"][:-4]}\n'
    ...
    elif id_cmd.search(msg):
        _id = id_cmd.search(msg)
        # 去除id，获得女武神名称
        name = id_cmd.sub('', msg)
        for k, v in a_list.items():
            # 获取女武神答案对应标准名称
            if name in v:
                try:
                    ...
                    v_list = GameSession.__load__()["normal"][k]
                    # 获取对应id语音
                    voice = v_list[int(_id.group())]
                except KeyError:
                    return await getVoice.send("语音列表未生成或有错误，请先发送‘更新崩坏三语音列表’来更新")
                    # 发送语音
                v_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'assets/record', voice['voice_path'])
                voice_path = f"file:///{v_file}"
...
# 崩坏三语音新增答案
@addAnswer.handle()
async def add_answer(arg: Message = CommandArg()):
    msg = arg.extract_plain_text().strip()
    if not msg:
        await addAnswer.finish("未输入答案", at_sender=True)
    msg_arr = msg.replace("：", ":").split(":")
    # 答案格式判断
    if len(msg_arr) != 2:
        await addAnswer.finish("答案有误，输入参考：崩坏三语音新增答案丽塔(标准答案):缭乱星棘(别称)", at_sender=True)
    origin = msg_arr[0].strip()
    new = msg_arr[1].strip()
    answer_data = GameSession.__load__("answer.json")
    if origin not in answer_data:
        await addAnswer.finish(f"{origin}不存在。")
    if new in answer_data[origin]:
        await addAnswer.finish("答案已存在。")
    # 新增答案
    answer_data[origin].append(new)
    # 保存答案
    with open(
...
async def _():
    url = "https://ghproxy.com/https://raw.githubusercontent.com" \
          "/MobiusT/zhenxun_extensive_plugin_mobius/main/bh3/guess_voice/answer_template.json"
    data_remote = await AsyncHttpx.get(url, follow_redirects=True)
...
async def update_voice_list():
    ...
    await undateVoice.finish(
        f"崩坏3语音列表更新完成，当前共有语音{num_hard + num_normal}条，其中普通{num_normal}条，困难{num_hard}条")
