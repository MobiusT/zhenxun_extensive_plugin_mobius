...
bind = on_command(
    "崩坏三绑定",
    aliases={"崩三绑定", "崩3绑定", "崩坏3绑定", "崩坏三绑定uid", "崩三绑定uid", "崩3绑定uid", "崩坏3绑定uid"},
    priority=5, block=True
)
server = on_command("崩坏三服务器列表",
                    aliases={"崩三服务器列表", "崩3服务器列表", "崩坏3服务器列表", "崩坏三服务器", "崩三服务器",
                             "崩3服务器", "崩坏3服务器"}, priority=5, block=True)
ck = on_command(
...
async def _(event: MessageEvent, arg: Message = CommandArg()):
    ...
    qid_db.set_uid_by_qid(str(qid), role_id.group())
    await bind.send("已绑定")
...
async def _():
    ...
    for server_id, alias in region.items():
        server_str = f'{server_str}{alias["name"]}服: '
        for alias_server in alias["alias"]:
            server_str = f'{server_str}{alias_server}, '
        server_str = f'{server_str[:-2]}; \n'
    await server.send(server_str)
...
async def _(bot: Bot, event: MessageEvent, arg: Message = CommandArg()):
    ...
    ck_flag = bool(cookie or str(event.user_id) not in bot.config.superusers)
    qid = event.user_id
    if ck_flag:
        qid_db = DB("uid.sqlite", tablename="qid_uid")
        # 验证uid绑定状态
        try:
            qid_db.get_uid_by_qid(str(qid))
        except Exception as e:
            logger.error("未绑定uid", e=e)
            return await ck.send("请先绑定uid，如崩坏三绑定114514官服")
    try:
        # 同步真寻原神ck
        login_ticket = None
        account_id = None
        ...
        if msg == "扫码":
            ...
            msg_data = await ck.send(
                f'{MessageSegment.image(qrcode_img)}\n请在3分钟内使用米游社扫码并确认进行绑定。\n注意：\n'
                f'1.扫码即代表你同意将Cookie信息授权给真寻及{event.sender.card}\n'
                f'2.扫码时会提示登录崩坏三，实际不会顶号\n'
            ...
            if not login_ticket:
                if not cookie:
                    raise InfoError('尚未绑定原神cookie，请先绑定原神cookie或发送 崩坏三ck 绑定崩坏三ck')
                # 用: 代替=, ,代替;
        ...
        if login_ticket:
            ...
            bbs_cookie_url = f"https://webapi.account.mihoyo.com/Api/cookie_accountinfo_by_loginticket" \
            ...
            if "成功" in data["data"]["msg"]:
                ...
                raise InfoError('登录信息失效，请重新获取最新cookie进行绑定')
        else:
        ...
        spider = Finance(qid=str(qid), cookieraw=f"{account_id},{cookie_token}")
    except InfoError as e:
        return await ck.send(str(e))
    try:
    ...
    await ck.finish(f"已绑定{img}", at_sender=True)
...
async def get_cookie_token(game_token: dict):
    res = await AsyncHttpx.get(
        f"https://api-takumi.mihoyo.com/auth/api/getCookieAccountInfoByGameToken?"
        f"game_token={game_token['token']}&account_id={game_token['uid']}")
    ...
    try:
        ...
        if not ck_flag:
            send_msg = f"当前未配置查询ck，请在真寻配置文件config.yaml的bind_bh3.COOKIE下配置如下内容，然后重启真寻。" \
                       f"\ncookie_token={cookie_token};account_id={account_id}"
            return
        spider = Finance(qid=qrcode_data["user_id"], cookieraw=f"{account_id},{cookie_token}")
        try:
            # 绑定ck
            fi = await spider.get_finance()
        except InfoError as e:
        ...
        import traceback
        logger.error(f"崩三ck扫码检查出错：{traceback.format_exc()}")
        send_msg = f"{type(e)}: {str(e)}"
        scheduler.remove_job(f'bh3_check_qrcode_{qrcode_data["user_id"]}', )
    finally:
        # 反馈绑定结果
        if send_msg:
            if bot := get_bot():
                if ck_flag and "group_id" in qrcode_data:
                    await bot.send_group_msg(group_id=qrcode_data["group_id"],
                                             message=at(qrcode_data["user_id"]) + send_msg)
                else:
